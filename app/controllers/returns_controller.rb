...
class ReturnsController < PaperProcessesController
  def index
    paper_processes = PaperProcess.returns.where(
    ...
    respond_to do |format|
      ...
      format.csv do
        send_data(paper_processes.asc(:created_at).to_csv, type: 'text/csv', filename: "returns_#{@current_time.strftime('%Y_%m')}.csv")
      end
    end
  end
