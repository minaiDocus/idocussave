...
class Admin::DematboxFilesController < Admin::AdminController
  ...
  def index
    @dematbox_files = TempDocument.dematbox_scan.originals.where(dematbox_file_contains).order_by(sort_column => sort_direction).page(params[:page]).per(params[:per_page])
  end
