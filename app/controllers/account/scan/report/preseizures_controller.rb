# -*- encoding : UTF-8 -*-
class Account::Scan::Report::PreseizuresController < Account::AccountController
  layout nil
  ...
  def show
    respond_to do |format|
      format.html {}
      format.csv do
        send_data(@report.preseizures.to_csv, :type=> "text/csv", :filename => "#{@report.pack.name.gsub(' ','_').sub('_all','')}.csv")
      end
    end
  end
end
