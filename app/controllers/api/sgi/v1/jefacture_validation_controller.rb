...
class Api::Sgi::V1::JefactureValidationController < SgiApiController
	...
	def pre_assigned
		if params[:data_valided].present?
			results = SgiApiServices::AutoPreAssignedJefacturePieces.new(params[:data_valided]).execute
			...
			render json: { success: false, message: 'Paramètre data_valided manquant' }, status: 601
		end
	end
