...
module DocumentsHelper
  ...
  def tbody(items)
     content_tag :tbody do
       items.each_with_index do |(k, _v), index|
         concat(content_tag(:tr) do
                  concat content_tag :td, (index + 1).to_s
                  concat content_tag :td, l(k['date'].localtime).to_s
                  concat content_tag :td, (k['uploaded']).to_s, style: 'text-align:right'
                  concat content_tag :td, (k['scanned']).to_s, style: 'text-align:right'
                  concat content_tag :td, (k['dematbox_scanned']).to_s, style: 'text-align:right'
                  concat content_tag :td, (k['fiduceo']).to_s, style: 'text-align:right'
                end
               )
       end
     end
