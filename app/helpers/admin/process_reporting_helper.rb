...
module Admin::ProcessReportingHelper
  ...
  def sum_of_requested_kits(customers, time)
    if time < Time.local(2016)
      ...
      customer_ids = Period.where(user_id: customers.map(&:id)).where("created_at >= ? AND created_at <= ?", time.dup, time.end_of_month).select do |period|
      ...
      previous_count = Period.where(user_id: customer_ids).where("created_at >= ? AND created_at <= ?", previous_month, previous_month.end_of_month).select do |period|
        options = period.product_option_orders.select do |option|
          option.title.match(pattern) || (option.group_title.match(pattern) && option.title.match(/Oui/i))
        end
