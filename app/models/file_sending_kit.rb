...
class FileSendingKit < ApplicationRecord
  ...
  def real_logo_path
    normal_paper_set_order? ? CmsImage.get_path_of(logo_path) : self.cloud_center_logo_object.path
  end

  def real_left_logo_path
    normal_paper_set_order? ? CmsImage.get_path_of(left_logo_path) : self.cloud_left_logo_object.path
  end

  def real_right_logo_path
    normal_paper_set_order? ? CmsImage.get_path_of(right_logo_path) : self.cloud_right_logo_object.path
  end
