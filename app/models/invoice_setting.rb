class InvoiceSetting < ApplicationRecord
  ...
  def self.invoice_synchronize(period, invoice_setting, organization)
    invoices_to_synchonize = Invoice.where('created_at >= ? AND created_at <= ? AND organization_id = ?', period, Time.now, organization.id)

    invoices_to_synchonize.each do |invoice_to_synchonize|
