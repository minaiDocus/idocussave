class Sftp < ApplicationRecord
  ...
  def used?
    external_file_storage ? external_file_storage.is_used?(ExternalFileStorage::F_SFTP) : true
  end

  def enable
    external_file_storage ? external_file_storage.use(ExternalFileStorage::F_SFTP) : false
  end

  def disable
    external_file_storage ? external_file_storage.unuse(ExternalFileStorage::F_SFTP) : false
  end
  ...
  def host_format
    ...
    valid_format = host.match(/\Asftp:\/\/(([a-z0-9]+([\-\.]{1}[a-z0-9]+)*\.[a-z]{2,5})|localhost)(\/)?\z/ix) ||
