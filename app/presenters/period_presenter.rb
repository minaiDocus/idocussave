...
class PeriodPresenter
  ...
  def render_json
    ...
    hash.merge!({ options: options_json }) if can_display_options?
    hash
  end

  def can_display_options?
    @viewer.is_admin ||
    (@viewer.is_prescriber && @viewer.customers.include?(@user)) ||
  ...
  def documents_json
...

    {
      ...
      delivery: @period.delivery.state
    }
  end
  ...
  def options_json
    ...
    if @period.organization || @period.user
      ...
      _invoices = (@period.organization || @period.user).invoices.where(:created_at.gte => start_time, :created_at.lte => end_time)
