...
class SgiApiServices::AutoPreAssignedJefacturePiecesValidation
  ...
  def check_in_accounting_plan(third_party_account)
    found = @piece.user.accounting_plan.providers.where(third_party_account: third_party_account).first
    found = @piece.user.accounting_plan.customers.where(third_party_account: third_party_account).first if not found
  end
end