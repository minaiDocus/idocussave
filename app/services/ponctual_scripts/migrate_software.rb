    ...
    Interfaces::Software::Configuration::SOFTWARES.each do |software_name|
      ...
      if software_name == 'ibiza'
        ...
        if ibiza
          ...
          software.is_used                          = ibiza.used?
          software.auto_deliver                     = ibiza.is_auto_deliver == true ? 1 : 0
          ...
          software.is_analysis_activated            = ibiza.is_analysis_activated
          software.is_analysis_to_validate          = ibiza.is_analysis_to_validate
        ...
        if csv_descriptor
          ...
          software.auto_deliver                  = organization.try(:is_csv_descriptor_auto_deliver)
    ...
    Interfaces::Software::Configuration::SOFTWARES.each do |software_name|
      ...
      if software_name == 'ibiza' && user.try(:ibiza_id).try(:present?) && softwares_setting.try(:is_ibiza_used) && !softwares_setting.try(:is_exact_online_used)
        # Defaut values to avoid exception validation on (state, state_2, and voucher_ref_target)
          ...
          software.auto_deliver                     = softwares_setting.try(:is_ibiza_auto_deliver)
          software.is_analysis_activated            = softwares_setting.try(:is_ibiza_compta_analysis_activated)
          software.is_analysis_to_validate          = softwares_setting.try(:is_ibiza_analysis_to_validate)
        ...
        if exact_online
          ...
          software.auto_deliver            = softwares_setting.try(:is_exact_online_auto_deliver)
        ...
        if csv_descriptor
          ...
          software.auto_deliver                  = softwares_setting.try(:is_csv_descriptor_auto_deliver)
        ...
        software.auto_deliver = softwares_setting.send("is_#{software_name}_auto_deliver".to_sym)
      end
