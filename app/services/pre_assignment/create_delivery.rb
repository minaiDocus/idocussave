...
class PreAssignment::CreateDelivery
  ...
  def valid_my_unisoft?
    @preseizures.any? && @report.try(:organization).try(:my_unisoft).try(:organization_used) && (!@is_auto || @report.user.my_unisoft.customer_auto_deliver) && !@preseizures.select(&:is_locked).first
  end
