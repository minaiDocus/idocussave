  ...
  def execute
    ...
    temp_documents.each do |temp_document_info|
      ...
      if raw_temp_documents.size > 1
        _temp_document = raw_temp_documents.group_by{|element| [element[:original_file_name], element[:pages_number]]}.map{|key, value| key + [value.map{|p| [p.created_at.strftime('%d-%m-%Y %H:%M:%S'), p.api_name, p.content_file_name, p.original_fingerprint]}]}.first
        tmp_doc_elements = []
        if _temp_document[0].present? && _temp_document[2].size > 1
          _temp_document[2].each{|tmp_doc_el| tmp_doc_elements << tmp_doc_el.join(' => ')}
          count += 1
          messages << "user_code: #{raw_temp_documents.first.user.code}, original_file_name: #{_temp_document[0]}, count: #{_temp_document[2].size}, pages_number: #{_temp_document[1]}, details: #{tmp_doc_elements.join('<br/>')}"
        end
      end
    end
