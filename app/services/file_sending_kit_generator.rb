...
class FileSendingKitGenerator
  TEMPDIR_PATH = "#{Rails.root}/files/#{Rails.env}/kit/".freeze

  def self.generate(clients_data, file_sending_kit, one_workshop_labels_page_per_customer = false)
    BarCode.init
  ...
  def self.to_clients(clients_data)
    clients_data.map { |client_data| client_data[:user] }
  end


  def self.to_folders(clients_data)
    folders_data = []
  ...
  def self.to_folder(client_data, period, account_book_type)
    part_number = 1
  ...
  def self.to_mails(users)
    users.map { |user| to_mail(user) }
  end


  def self.to_mail(user)
    mail = {}
  ...
  def self.to_labels(clients_data, paper_set_shipping = false)
    labels = []
  ...
  def self.to_label(client_data, paper_set_shipping = false)
    user = client_data[:user]

    address = if paper_set_shipping
                    user.paper_set_shipping_address
                  else
                    user.paper_return_address
                  end
