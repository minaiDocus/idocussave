class DropboxImport
  ...
  def process_entry(path, metadata)
    if metadata.is_a?(Hash)
      ...
      folder_states.each do |folder_state|
        next unless folder_state.first.downcase =~ /\A#{Regexp.quote(path)}/
  ...
  def remove_folders
    unused_folder_paths = folder_states.select { |_path, state| state.nil? }.map(&:first)
    ...
    if user.is_prescriber
      ...
      unused_folder_paths.each do |unused_folder_path|
        ...
        paths.each do |path|
          if unused_folder_path =~ /\A#{path}/
            is_already_added = true

            break
          end
        end
        next if is_already_added
  ...
  def add_folders
    new_folder_paths = folder_states.select { |_path, state| state == false }.map(&:first)

    new_folder_paths.uniq.each do |new_folder_path|
      begin
        client.file_create_folder new_folder_path
      rescue DropboxError => e
        raise unless e.message =~ /folder already exists/
      end
    end
  end
end
