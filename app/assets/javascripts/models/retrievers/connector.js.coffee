class Idocus.Models.Connector extends Backbone.Model
  ...
  contact_fields: ()->
    ...
    @common.fields_constructor(fields, this.is_disabled_fields())
  ...
  is_disabled_fields: () ->
    ...
    if window.location.pathname == '/account/retrievers/new' && ( @get('capabilities').includes('document') && !@get('capabilities').includes('bank') )
      disabled = true
