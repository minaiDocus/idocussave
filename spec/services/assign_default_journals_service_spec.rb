...
describe AssignDefaultJournalsService do
  describe '#execute' do
    ...
    context 'when collaborator is the source' do
      before(:all) do
        @organization.update_attribute(:is_journals_management_centralized, false)
      end
      ...
      context 'given 3 default journals' do
        before(:all) do
          ...
          @collaborator.account_book_types << @journal1
          @collaborator.account_book_types << @journal2
          @collaborator.account_book_types << @journal3
        end
      ...
      context 'given 1 default journal with pre assignment' do
        before(:all) do
          ...
          @journal.save
          @collaborator.account_book_types << @journal
        end
    ...
    context 'when organization is the source' do
      before(:all) do
        @organization.update_attribute(:is_journals_management_centralized, true)
        @journal = AccountBookType.create(name: 'AC', description: '(Achat)', is_default: true)
